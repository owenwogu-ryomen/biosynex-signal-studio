...
class SignalStudioApp:
    ...
    def _build_right_panel(self):
        ...
        self.plot_button = ttk.Button(
            toolbar,
            text="Plot Selected Column",
            command=self.plot_selected_column,
            style="Primary.TButton"
        )

        self.plot_button.pack(side="right", padx=(8, 0))
        ...
        self.graph_frame = ttk.Frame(self.right_panel)
    ...
    def _update_preview_button_state(self):
        ...
        if pd.api.types.is_numeric_dtype(self.df[self.selected_column]):
            self.plot_button.state(["!disabled"])
        else:
            self.plot_button.state(["disabled"])
    ...
    def plot_selected_column(self):
        if self.df is None or self.selected_column is None:
            self.status_var.set("No column selected for plotting.")
            return

        start = self.preview_start
        end = start + self.preview_size

        data = (
            self.df[self.selected_column]
            .iloc[start:end]
            .dropna()
        )

        if data.empty:
            self.status_var.set(f"No data available in column '{self.selected_column}' to plot.")
            return
        
        if not pd.api.types.is_numeric_dtype(data):
            self.status_var.set(f"'{self.selected_column}' is not a numeric column.")
            return
        ...
        ax.set_title(self.selected_column)
        ...
        self.status_var.set(f"Plotted column '{self.selected_column}' successfully.")
